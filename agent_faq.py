...
# Import AWS Bedrock SDK or client
import boto3
...
def getFinanceSummary(customerId: int) -> str:
    ...
    finance = [
        {"customerId": 123, "balance": 1000, "lastTransaction": "2021-01-01"},
        {"customerId": 124, "balance": 2000, "lastTransaction": "2021-01-01"},
    ]
    for f in finance:
...
def getCustomerDetails(customerId: int) -> str:
    ...
    customer = [
        {"customerId": 123, "name": "John Doe", "email": "john.doe@gmail.com"},
        {"customerId": 124, "name": "Jane Doe", "email": "jane.doe@gmail.com"},
    ]
    for c in customer:
...
# Define a function to call AWS Bedrock
def call_bedrock(model, input_text):
    response = bedrock_client.invoke_model(
        ModelId=model, ContentType="application/json", Body=input_text
    )
    return response["Body"].read().decode("utf-8")


# Bind tools to the AWS Bedrock model
llm = call_bedrock(model="your-bedrock-model-id", input_text="").bind_tools(tools)
...
prompt.messages[0] = SystemMessagePromptTemplate(
    prompt=PromptTemplate(
        ...
        partial_variables={},
        template=(
            "You are a banking assistant. Respond to customer queries accurately using the tools at your disposal. Use the tools to assist customers regarding their spending summary, complaints, recent transactions with bank\n"
